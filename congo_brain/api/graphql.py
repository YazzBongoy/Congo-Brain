...
class Query:
    ...
    def dashboard(self) -> Dashboard:
        e = _get_engine()
        d = e.get_dashboard()
        return Dashboard(model=d["model"], formula=d["formula"], snn=d["snn"], entity_counts=d["entity_counts"])
    ...
    def snn(self) -> SNNAggregate:
        ...
        return SNNAggregate(
            ...
            total_ps=a.total_ps,
            total_gr=a.total_gr,
            total_nrv=a.total_nrv,
            total_dwl=a.total_dwl,
            total_ec=a.total_ec,
            snn=a.snn,
            snn_rate=a.snn_rate,
        )
        ...
        for c in e.companies:
            ...
            result.append(
                Company(
                    ...
                    tax_burden=c.get("tax_burden", 0),
                    corruption_cost=c.get("corruption_cost", 0),
                    ps_value=round(ps, 2),
                )
            )
        return result
    ...
    def projects(self) -> list[Project]:
        ...
        return [
            Project(
                ...
                cost=p.get("cost", 0),
                snn_contribution=p.get("snn_contribution", 0),
                cs_impact=p.get("cs_impact", 0),
                ps_impact=p.get("ps_impact", 0),
    ...
    def public_services(self) -> list[PublicService]:
        ...
        for ps in e.public_services:
            ...
            result.append(
                PublicService(
                    name=ps["name"],
                    quality_score=ps.get("quality_score", 0),
    ...
    def scenarios(self) -> list[Scenario]:
        ...
        return [
            Scenario(
                ...
                cs_growth=s.cs_growth,
                ps_growth=s.ps_growth,
...
def build_schema(environment: str) -> strawberry.Schema:
    """Build a schema with production introspection disabled."""
    extensions = [strawberry.extensions.DisableIntrospection] if environment in {"production", "staging"} else []
    return strawberry.Schema(query=Query, extensions=extensions)


from congo_brain.core.config import ENVIRONMENT  # noqa: E402

schema = build_schema(ENVIRONMENT)
