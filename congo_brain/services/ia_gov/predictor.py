...
SCENARIOS = {
    "baseline": Scenario(
        ...
        cs_growth=0.02, ps_growth=0.015, gr_growth=0.025,
        nrv_growth=0.03, dwl_reduction=0.01, ec_reduction=0.005,
        volatility=0.03,
    ),
    "investissement_minier": Scenario(
        ...
        cs_growth=0.03, ps_growth=0.08, gr_growth=0.05,
        nrv_growth=0.10, dwl_reduction=0.02, ec_reduction=-0.02,
        annual_investment=2000, investment_snn_multiplier=2.0,
        volatility=0.06,
    ),
    "reforme_fiscale": Scenario(
        name="Réforme fiscale",
        description="Élargissement assiette, réduction évasion, TSCA",
        cs_growth=0.02, ps_growth=0.03, gr_growth=0.08,
        nrv_growth=0.03, dwl_reduction=0.05, ec_reduction=0.01,
        volatility=0.04,
    ),
    "anti_corruption": Scenario(
        name="Lutte anti-corruption",
        description="Transparence, e-procurement, audit renforcé",
        cs_growth=0.03, ps_growth=0.04, gr_growth=0.06,
        nrv_growth=0.03, dwl_reduction=0.10, ec_reduction=0.02,
        volatility=0.04,
    ),
    "transition_verte": Scenario(
        name="Transition verte",
        description="Énergies renouvelables, réduction EC, tourisme",
        cs_growth=0.04, ps_growth=0.02, gr_growth=0.03,
        nrv_growth=0.01, dwl_reduction=0.03, ec_reduction=0.08,
        annual_investment=1500, investment_snn_multiplier=1.8,
        volatility=0.05,
    ),
    "optimiste": Scenario(
        name="Scénario optimiste",
        description="Toutes les réformes combinées",
        cs_growth=0.05, ps_growth=0.07, gr_growth=0.08,
        nrv_growth=0.06, dwl_reduction=0.08, ec_reduction=0.06,
        annual_investment=3000, investment_snn_multiplier=2.2,
        volatility=0.04,
    ),
    "pessimiste": Scenario(
        name="Scénario pessimiste",
        description="Instabilité politique, chute cours matières premières",
        cs_growth=-0.02, ps_growth=-0.03, gr_growth=-0.01,
        nrv_growth=-0.05, dwl_reduction=-0.02, ec_reduction=0.01,
        volatility=0.08,
    ),
}
...
class PredictiveModel:
    ...
    def set_baseline(self, cs: float, ps: float, gr: float,
                     nrv: float, dwl: float, ec: float) -> None:
        self.base_cs = cs
        self.base_ps = ps
        self.base_gr = gr
        self.base_nrv = nrv
    ...
    def base_snn(self) -> float:
        return (self.base_cs + self.base_ps + self.base_gr
                + self.base_nrv - self.base_dwl - self.base_ec)

    def project(self, scenario: Scenario, years: int = 10,
                monte_carlo_runs: int = 100) -> PredictionResult:
        ...
        return PredictionResult(
            ...
            snn_change_pct=round((final.snn - self.base_snn) / abs(self.base_snn) * 100, 1) if self.base_snn != 0 else 0,
    ...
    def compare_scenarios(self, years: int = 10) -> dict:
        ...
        return {
            ...
            "ranking": sorted(
                ...
                key=lambda x: x["final_snn"], reverse=True,
            ),
        }
    ...
    def _deterministic(self, scenario: Scenario, years: int) -> list[YearProjection]:
        ...
        for y in range(1, years + 1):
            ...
            projections.append(YearProjection(
                year=y, cs=cs, ps=ps, gr=gr, nrv=nrv,
                dwl=dwl, ec=ec, snn=snn, investment_snn=inv_snn,
            ))
