...
class GEOSReport(FPDF):
    ...
    def data_table(self, headers: list[str], rows: list[list[str]], col_widths: list[float] | None = None) -> None:
...
def generate_snn_pdf(engine, predictions: dict | None = None) -> bytes:
    ...
    pdf.data_table(["Composant", "Valeur (M USD)", "Contribution"], data, [70, 50, 50])

    # Entities
    pdf.section_title("2. Entités GEOS")
    entity_data = [
        ["Provinces", str(len(engine.provinces)), ", ".join(p["name"] for p in engine.provinces[:3])],
        ["Entreprises", str(len(engine.companies)), ", ".join(c["name"] for c in engine.companies[:3])],
        ["Ressources", str(len(engine.resources)), ", ".join(r["name"] for r in engine.resources[:3])],
        ["Ministères", str(len(engine.ministries)), ", ".join(m["name"] for m in engine.ministries[:3])],
        ["Impôts", str(len(engine.taxes)), ", ".join(t["name"] for t in engine.taxes[:3])],
        ["Projets", str(len(engine.projects)), ", ".join(p["name"] for p in engine.projects[:3])],
        ["Indicateurs", str(len(engine.indicators)), "Macro, Budget, Fiscal"],
    ]
    pdf.data_table(["Entité", "Nombre", "Exemples"], entity_data, [40, 20, 110])
    ...
    if predictions and "projections" in predictions:
        ...
        pdf.data_table(["Année", "SNN", "CS", "PS", "NRV"], proj_data, [25, 40, 40, 40, 40])
        ...
        pdf.cell(
            0,
            8,
            f"SNN final moyen (Monte Carlo): {predictions['mean_final_snn']:,.0f} M USD",
            new_x="LMARGIN",
            new_y="NEXT",
        )
        pdf.cell(
            0,
            8,
            f"Intervalle 95%: [{predictions['ci_5']:,.0f} - {predictions['ci_95']:,.0f}] M USD",
            new_x="LMARGIN",
            new_y="NEXT",
        )
...
def generate_snn_excel(engine, predictions: dict | None = None) -> bytes:
    ...
    for p in engine.provinces:
        ws2.append(
            [
                ...
                p.get("population", 0),
                p.get("area_km2", 0),
                p.get("literacy_rate", 0),
                p.get("internet_pct", 0),
    ...
    for c in engine.companies:
        ...
        ws3.append([c["name"], c.get("sector", ""), c.get("revenue", 0), tc, round(ps, 2)])
    ...
    for r in engine.resources:
        ...
        ws4.append(
            [
                r["name"],
                r.get("type", ""),
                r.get("annual_production_tons", 0),
                r.get("market_value_per_ton", 0),
                r.get("local_processing_pct", 0),
                round(nrv, 2),
                r.get("environmental_cost", 0),
            ]
        )
    ...
    if predictions and "projections" in predictions:
        ...
        for p in predictions["projections"]:
            ws6.append([p["year"], p["snn"], p["cs"], p["ps"], p["gr"], p["nrv"], p["dwl"], p["ec"]])
