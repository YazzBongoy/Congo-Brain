...
from congo_brain.services.ia_gov.snn_engine import SNNAggregate, SNNOptimizationEngine
...
class TestSNNAggregate:
    ...
    def test_snn_positive(self):
        agg = SNNAggregate(total_cs=10, total_ps=20, total_gr=15, total_nrv=5, total_dwl=3, total_ec=2)
        assert agg.snn == 45
    ...
    def test_to_dict_structure(self):
        agg = SNNAggregate(
            total_cs=10, total_ps=20, total_gr=15, total_nrv=5, total_dwl=3, total_ec=2, cs_sources={"a": 10}
        )
        d = agg.to_dict()
        assert "snn" in d
...
class TestSNNOptimizationEngine:
        ...
        engine.resources = [
            {
                ...
                "market_value_per_ton": 10000,
                "local_processing_pct": 20,
                "tax_rate": 30,
    ...
    def test_compute_snn_from_public_services(self):
        ...
        engine.public_services = [
            {"name": "Santé", "willingness_to_pay": 30, "actual_price": 8, "quality_score": 4, "indirect_cost": 12}
        ]
        agg = engine.compute_snn()
    ...
    def test_compute_snn_from_taxes(self):
        engine = SNNOptimizationEngine()
        engine.taxes = [{"name": "TVA", "revenue": 500, "evasion_estimate": 100}]
        agg = engine.compute_snn()
    ...
    def test_compute_snn_from_corruption(self):
        engine = SNNOptimizationEngine()
        engine.companies = [{"name": "C1", "revenue": 0, "corruption_cost": 50}]
        agg = engine.compute_snn()
        assert agg.dwl_sources["corruption_entreprises"] == 50

    def test_compute_snn_from_contracts(self):
        engine = SNNOptimizationEngine()
        engine.contracts = [{"name": "C1", "value": 100, "corruption_flag": True}]
        agg = engine.compute_snn()
    ...
    def test_snn_formula_all_components(self):
        engine = SNNOptimizationEngine()
        engine.public_services = [
            {"name": "E", "willingness_to_pay": 10, "actual_price": 5, "quality_score": 10, "indirect_cost": 0}
        ]  # CS=5
        engine.companies = [
            {
                "name": "C",
        ...
        engine.resources = [
            {
                "name": "R",
                "annual_production_tons": 100,
                "market_value_per_ton": 10000,
                "local_processing_pct": 0,
                "tax_rate": 0,
                "environmental_cost": 5,
            }
        ]  # NRV=1, EC=5
        agg = engine.compute_snn()
    ...
    def test_optimize_allocation_budget_constraint(self):
        ...
        assert "P3" not in result["allocations"]
        assert all(allocation["fraction"] > 0 for allocation in result["allocations"].values())
...
def test_geos_requires_authentication(client):
    assert client.get("/api/v1/geos/dashboard").status_code == 401
